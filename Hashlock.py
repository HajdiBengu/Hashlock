...
def main():
    class Hashlock(sp.Contract):
        ...
        def commit(self, receiver, amount, hash):
            assert sp.source == self.data.sender, "Not Admin"
        ...
        def reveal(self, secretKey):
            sp.cast(secretKey, sp.nat)
            assert sp.now >= self.data.revealTime, "5 mins need to pass"
            assert self.data.revealed == False, "Already revealed"
            ...
            assert self.data.saltedHash == revealHash, "Didn't match"
            self.data.revealed = True
...
def test():
    ...
    r.commit(receiver=bob.address, amount=amount, hash=hashInput).run(source=alice.address, amount=wrong_amount, now=sp.timestamp_from_utc_now(), valid=False)
    ...
    r.commit(receiver=bob.address, amount=amount, hash=hashInput).run(source=mallory.address, amount=amount, now=sp.timestamp_from_utc_now(), valid=False)

    s.h3("Admin calls commit twice")
    r.commit(receiver=bob.address, amount=amount, hash=hashInput).run(source=alice.address, amount=amount, now=sp.timestamp_from_utc_now(), valid=False)

    s.h3("Receiver cannot reveal unless 5 minutes have passed")
    r.reveal(secret_key).run(source=bob.address, now=sp.timestamp_from_utc_now(), valid=False)
    ...
    r.reveal(secret_key).run(source=bob.address, now=sp.timestamp_from_utc_now().add_seconds(600), valid=False)
